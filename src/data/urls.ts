...
export const Urls: any = {
  ...
  locationSuggetionUrl: (query: String) =>
    `/autocomplete?key=${process.env.REACT_APP_LOCATION_IQ_KEY}&q=${query}&format=json&countrycodes=in&limit=5`,
  findAddressUrl:(loc:Location)=>`/reverse?key=${process.env.REACT_APP_LOCATION_IQ_KEY}&lat=${loc?.lat}&lon=${loc?.lon}&format=json`
};
