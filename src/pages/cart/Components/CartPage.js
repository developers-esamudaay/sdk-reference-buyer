...
const CartPage=()=>{
    ...
    const totalPices=cartData?.items?.reduce((val,item)=>val+item?.quantity?.count,0)
    return (
        <>
      <Navbar/>
        <div className={styles.cart_container}>
          {
            cartData?.items?.length===0?(<div className={styles.empty_cart_container}>
              <div>
                <p className={styles.empty_cart_text}>Cart Is Empty</p>
                </div>
              <img className={styles.empty_cart_image}src={empty_cart}/> <button onClick={()=>history.push("/products")} className={styles.go_to_products_button}>Go To Products</button></div>):(<>
              <div className={styles.cart_header}>
                <p className={styles.header_text}>Bag <span>{`${cartData?.items?.length} items|${totalPices} pices`}</span></p>
